...
static void init_jni_cache_once(void) {
    ...
    jclass local = (*env)->FindClass(env, "com/metre/Metre$PollBatch");
    if (local == NULL) {
        return;
    }
    ...
    local = (*env)->FindClass(env, "com/metre/Metre$PollResult");
    if (local == NULL) {
        return;
    }
    ...
    JNI_CACHE.poll_result_ctor = (*env)->GetMethodID(
        ...
        "<init>",
        "(ILcom/metre/Metre$PollBatch;)V"
    );
    if (JNI_CACHE.poll_result_ctor == NULL) {
        return;
    }

    local = (*env)->FindClass(env, "com/metre/Metre$RuleMetadata");
    if (local == NULL) {
        return;
    }
...
static void call_java_log_handler(
    ...
    jstring target_str = (*env)->NewStringUTF(env, target ? target : "metre");
...
JNIEXPORT jlong JNICALL Java_com_metre_Metre_nativeCreateEngine(
    JNIEnv *env,
    jclass cls,
    jstring path,
    jint payload_format,
    ...
    void *engine = metre_create_engine(
        path_str,
        (int)payload_format,
        client_ids_path_str,
        (int)notify_mode,
...
JNIEXPORT void JNICALL Java_com_metre_Metre_nativeDestroy(JNIEnv *env, jclass cls, jlong handle) {
    (void)env;
    (void)cls;
    if (handle != 0) {
        clear_pending_batch();
        metre_destroy((void *)handle);
    }
}

JNIEXPORT jint JNICALL Java_com_metre_Metre_nativeDisconnect(JNIEnv *env, jclass cls, jlong handle) {
    (void)env;
    (void)cls;
    if (handle == 0) {
        return METRE_ERR_INVALID_ARGUMENT;
    }
    return metre_disconnect((void *)handle);
}

JNIEXPORT jint JNICALL Java_com_metre_Metre_nativeStartMqtt(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jstring host,
...
JNIEXPORT jlong JNICALL Java_com_metre_Metre_nativeRegisterLogHandler(
    JNIEnv *env,
    jclass cls,
    jobject handler) {
    (void)cls;
...
JNIEXPORT jint JNICALL Java_com_metre_Metre_nativeSetLogCallback(
    JNIEnv *env,
    jclass cls,
...
JNIEXPORT void JNICALL Java_com_metre_Metre_nativeFreeLogHandler(
    JNIEnv *env,
    jclass cls,
    jlong callback_handle) {
    (void)cls;
...
JNIEXPORT jlongArray JNICALL Java_com_metre_Metre_nativeMetricsSnapshotValues(
    JNIEnv *env,
    jclass cls,
    jlong handle) {
    (void)cls;
    ...
    struct MetreMetricsSnapshot snapshot;
    if (metre_metrics_snapshot((void *)handle, &snapshot) != 0) {
        return NULL;
    }
    jlong values[] = {
...
JNIEXPORT jint JNICALL Java_com_metre_Metre_nativeSetPollBatchLimit(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jint limit) {
    ...
    return metre_set_poll_batch_limit((void *)handle, (uint32_t)limit);
}

JNIEXPORT jint JNICALL Java_com_metre_Metre_nativeSetDetailedLatencyMetrics(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jboolean enabled) {
    ...
    return metre_set_detailed_latency_metrics((void *)handle, enabled);
}

JNIEXPORT jobjectArray JNICALL Java_com_metre_Metre_nativeGetRuleMetadataTable(
    JNIEnv *env,
    jclass cls,
    jlong handle) {
    (void)cls;
    ...
    struct MetreRuleMetadata *metadata = NULL;
    size_t metadata_len = 0;
    if (metre_get_rule_metadata_table((void *)handle, &metadata, &metadata_len) != 0) {
        return NULL;
    }
    if (!ensure_jni_cache(env)) {
        metre_free_rule_metadata_table(metadata, metadata_len);
        return NULL;
    }
    ...
    for (size_t i = 0; i < metadata_len; i++) {
        struct MetreRuleMetadata *record = &metadata[i];
