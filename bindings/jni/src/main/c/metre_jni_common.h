#ifndef METRE_JNI_COMMON_H
#define METRE_JNI_COMMON_H
...
struct MetreRuleMetadata {
    uint16_t rule_index;
    char **destinations;
    size_t destinations_len;
};

struct MetreMetricsSnapshot {
...
extern void *metre_create_engine(
    const char *path,
    int payload_format,
    const char *client_ids_path,
    int notify_mode,
    const char *protobuf_descriptor_set_path);
extern void metre_destroy(void *engine);
extern int metre_disconnect(void *engine);
extern int metre_start_mqtt(
    void *engine,
    const char *host,
    uint16_t port,
    const char *node_id,
...
extern int metre_poll_eval_results_packed(
    void *engine,
    uint32_t timeout_millis,
    struct MetrePackedEvalResults *out_results);
extern void metre_free_packed_eval_results(struct MetrePackedEvalResults *results);
extern int metre_get_rule_metadata_table(
    void *engine,
    struct MetreRuleMetadata **out_metadata,
    size_t *out_len);
extern void metre_free_rule_metadata_table(struct MetreRuleMetadata *metadata, size_t len);
extern int metre_set_log_callback(
    void (*callback)(void *, int, const char *, const char *, uint64_t, const char *, const char *, const char *, uint32_t),
    void *user_data,
    int min_level);
extern int metre_metrics_snapshot(
    void *engine,
    struct MetreMetricsSnapshot *out_snapshot);
extern int metre_set_poll_batch_limit(void *engine, uint32_t limit);
extern int metre_set_detailed_latency_metrics(void *engine, jboolean enabled);
...
static const int METRE_OK = 0;
static const int METRE_POLL_RESULT_READY = 1;
static const int METRE_ERR_INVALID_ARGUMENT = -1;
static const int METRE_ERR_INVALID_STATE = -2;
static const int METRE_ERR_INVALID_PARAMETER = -3;
static const int METRE_ERR_START_FAILED = -4;
static const int METRE_ERR_ALREADY_STARTED = -5;
static const int METRE_ERR_WORKER_UNAVAILABLE = -6;
static const int METRE_ERR_INTERNAL_QUEUE_ERROR = -7;
