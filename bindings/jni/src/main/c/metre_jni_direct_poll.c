...
struct DirectPendingBatch {
    struct MetrePackedEvalResults results;
    size_t next_index;
...
void clear_pending_batch(void) {
    if (DIRECT_PENDING_ACTIVE) {
        metre_free_packed_eval_results(&DIRECT_PENDING.results);
...
static int copy_batch_to_direct_buffers(
    const struct MetrePackedEvalResults *results,
    size_t *next_index,
    uint32_t *header_ptr,
    size_t header_capacity_records,
...
static int poll_results_batch_direct_core(
    ...
    if (pending != NULL) {
        ...
        if (merge_pending && emitted < header_capacity_records && payload_offset < payload_capacity_bytes) {
            struct MetrePackedEvalResults fetched = {0};
            int fetch_rc = metre_poll_eval_results_packed(engine, 0, &fetched);
            if (fetch_rc > 0 && fetched.len > 0) {
                ...
                if (rc < METRE_OK && emitted == 0) {
                    metre_free_packed_eval_results(&fetched);
                    return rc;
                }
    ...
    struct MetrePackedEvalResults fetched = {0};
    int rc = metre_poll_eval_results_packed(engine, timeout_millis, &fetched);
    if (rc < METRE_OK || fetched.len == 0) {
        if (fetched.len > 0) {
            metre_free_packed_eval_results(&fetched);
        }
        return rc;
...
JNIEXPORT jint JNICALL Java_com_metre_Metre_nativePollResultsBatchDirect(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jint timeout_millis,
