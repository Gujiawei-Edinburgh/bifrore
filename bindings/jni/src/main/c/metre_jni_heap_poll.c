...
JNIEXPORT jobject JNICALL Java_com_metre_Metre_nativePollResultsBatch(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jint timeout_millis) {
    ...
    if (handle == 0) {
        return (*env)->NewObject(
            ...
            (jint)METRE_ERR_INVALID_ARGUMENT,
            NULL
        );
    }

    struct MetrePackedEvalResults results = {0};
    int rc = metre_poll_eval_results_packed(
        (void *)handle,
        (uint32_t)timeout_millis,
        &results);
    if (rc < METRE_OK || results.len == 0) {
        return (*env)->NewObject(
            env,
            JNI_CACHE.poll_result_cls,
            JNI_CACHE.poll_result_ctor,
