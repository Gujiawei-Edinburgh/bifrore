...
static void init_jni_cache_once(void) {
    ...
    jclass local = (*env)->FindClass(env, "com/tenon/Tenon$PollBatch");
    if (local == NULL) {
        return;
    }
    ...
    local = (*env)->FindClass(env, "com/tenon/Tenon$PollResult");
    if (local == NULL) {
        return;
    }
    ...
    JNI_CACHE.poll_result_ctor = (*env)->GetMethodID(
        ...
        "<init>",
        "(ILcom/tenon/Tenon$PollBatch;)V"
    );
    if (JNI_CACHE.poll_result_ctor == NULL) {
        return;
    }

    local = (*env)->FindClass(env, "com/tenon/Tenon$RuleMetadata");
    if (local == NULL) {
        return;
    }
...
static void call_java_log_handler(
    ...
    jstring target_str = (*env)->NewStringUTF(env, target ? target : "tenon");
...
JNIEXPORT jlong JNICALL Java_com_tenon_Tenon_nativeCreateEngine(
    JNIEnv *env,
    jclass cls,
    jstring path,
    jint payload_format,
    ...
    void *engine = tenon_create_engine(
        path_str,
        (int)payload_format,
        client_ids_path_str,
        (int)notify_mode,
...
JNIEXPORT void JNICALL Java_com_tenon_Tenon_nativeDestroy(JNIEnv *env, jclass cls, jlong handle) {
    (void)env;
    (void)cls;
    if (handle != 0) {
        clear_pending_batch();
        tenon_destroy((void *)handle);
    }
}

JNIEXPORT jint JNICALL Java_com_tenon_Tenon_nativeDisconnect(JNIEnv *env, jclass cls, jlong handle) {
    (void)env;
    (void)cls;
    if (handle == 0) {
        return TENON_ERR_INVALID_ARGUMENT;
    }
    return tenon_disconnect((void *)handle);
}

JNIEXPORT jint JNICALL Java_com_tenon_Tenon_nativeStartMqtt(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jstring host,
...
JNIEXPORT jlong JNICALL Java_com_tenon_Tenon_nativeRegisterLogHandler(
    JNIEnv *env,
    jclass cls,
    jobject handler) {
    (void)cls;
...
JNIEXPORT jint JNICALL Java_com_tenon_Tenon_nativeSetLogCallback(
    JNIEnv *env,
    jclass cls,
...
JNIEXPORT void JNICALL Java_com_tenon_Tenon_nativeFreeLogHandler(
    JNIEnv *env,
    jclass cls,
    jlong callback_handle) {
    (void)cls;
...
JNIEXPORT jlongArray JNICALL Java_com_tenon_Tenon_nativeMetricsSnapshotValues(
    JNIEnv *env,
    jclass cls,
    jlong handle) {
    (void)cls;
    ...
    struct TenonMetricsSnapshot snapshot;
    if (tenon_metrics_snapshot((void *)handle, &snapshot) != 0) {
        return NULL;
    }
    jlong values[] = {
...
JNIEXPORT jint JNICALL Java_com_tenon_Tenon_nativeSetPollBatchLimit(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jint limit) {
    ...
    return tenon_set_poll_batch_limit((void *)handle, (uint32_t)limit);
}

JNIEXPORT jint JNICALL Java_com_tenon_Tenon_nativeSetDetailedLatencyMetrics(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jboolean enabled) {
    ...
    return tenon_set_detailed_latency_metrics((void *)handle, enabled);
}

JNIEXPORT jobjectArray JNICALL Java_com_tenon_Tenon_nativeGetRuleMetadataTable(
    JNIEnv *env,
    jclass cls,
    jlong handle) {
    (void)cls;
    ...
    struct TenonRuleMetadata *metadata = NULL;
    size_t metadata_len = 0;
    if (tenon_get_rule_metadata_table((void *)handle, &metadata, &metadata_len) != 0) {
        return NULL;
    }
    if (!ensure_jni_cache(env)) {
        tenon_free_rule_metadata_table(metadata, metadata_len);
        return NULL;
    }
    ...
    for (size_t i = 0; i < metadata_len; i++) {
        struct TenonRuleMetadata *record = &metadata[i];
