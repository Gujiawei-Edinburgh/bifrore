#ifndef TENON_JNI_COMMON_H
#define TENON_JNI_COMMON_H
...
struct TenonRuleMetadata {
    uint16_t rule_index;
    char **destinations;
    size_t destinations_len;
};

struct TenonMetricsSnapshot {
...
extern void *tenon_create_engine(
    const char *path,
    int payload_format,
    const char *client_ids_path,
    int notify_mode,
    const char *protobuf_descriptor_set_path);
extern void tenon_destroy(void *engine);
extern int tenon_disconnect(void *engine);
extern int tenon_start_mqtt(
    void *engine,
    const char *host,
    uint16_t port,
    const char *node_id,
...
extern int tenon_poll_eval_results_packed(
    void *engine,
    uint32_t timeout_millis,
    struct TenonPackedEvalResults *out_results);
extern void tenon_free_packed_eval_results(struct TenonPackedEvalResults *results);
extern int tenon_get_rule_metadata_table(
    void *engine,
    struct TenonRuleMetadata **out_metadata,
    size_t *out_len);
extern void tenon_free_rule_metadata_table(struct TenonRuleMetadata *metadata, size_t len);
extern int tenon_set_log_callback(
    void (*callback)(void *, int, const char *, const char *, uint64_t, const char *, const char *, const char *, uint32_t),
    void *user_data,
    int min_level);
extern int tenon_metrics_snapshot(
    void *engine,
    struct TenonMetricsSnapshot *out_snapshot);
extern int tenon_set_poll_batch_limit(void *engine, uint32_t limit);
extern int tenon_set_detailed_latency_metrics(void *engine, jboolean enabled);
...
static const int TENON_OK = 0;
static const int TENON_POLL_RESULT_READY = 1;
static const int TENON_ERR_INVALID_ARGUMENT = -1;
static const int TENON_ERR_INVALID_STATE = -2;
static const int TENON_ERR_INVALID_PARAMETER = -3;
static const int TENON_ERR_START_FAILED = -4;
static const int TENON_ERR_ALREADY_STARTED = -5;
static const int TENON_ERR_WORKER_UNAVAILABLE = -6;
static const int TENON_ERR_INTERNAL_QUEUE_ERROR = -7;
