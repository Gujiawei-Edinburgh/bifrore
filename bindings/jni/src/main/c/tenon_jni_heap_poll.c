...
JNIEXPORT jobject JNICALL Java_com_tenon_Tenon_nativePollResultsBatch(
    JNIEnv *env,
    jclass cls,
    jlong handle,
    jint timeout_millis) {
    ...
    if (handle == 0) {
        return (*env)->NewObject(
            ...
            (jint)TENON_ERR_INVALID_ARGUMENT,
            NULL
        );
    }

    struct TenonPackedEvalResults results = {0};
    int rc = tenon_poll_eval_results_packed(
        (void *)handle,
        (uint32_t)timeout_millis,
        &results);
    if (rc < TENON_OK || results.len == 0) {
        return (*env)->NewObject(
            env,
            JNI_CACHE.poll_result_cls,
            JNI_CACHE.poll_result_ctor,
