...
public final class Metre implements AutoCloseable {
    ...
    public static final int METRE_OK = 0;
    public static final int METRE_ERR_INVALID_ARGUMENT = -1;
    public static final int METRE_ERR_INVALID_STATE = -2;
    public static final int METRE_ERR_INVALID_PARAMETER = -3;
    public static final int METRE_ERR_START_FAILED = -4;
    public static final int METRE_ERR_ALREADY_STARTED = -5;
    public static final int METRE_ERR_WORKER_UNAVAILABLE = -6;
    public static final int METRE_ERR_INTERNAL_QUEUE_ERROR = -7;
    ...
    public Metre(MetreOptions options) {
        this(
            options.mqtt.host,
            options.mqtt.port,
    ...
    private Metre(
        ...
        if (nativeSetPollBatchLimit(this.handle, Math.max(1, pollBatchLimit)) != METRE_OK) {
            nativeDestroy(this.handle);
            this.handle = 0;
            throw new IllegalStateException("Failed to configure poll batch limit");
        }
        if (nativeSetDetailedLatencyMetrics(this.handle, this.detailedLatencyMetrics) != METRE_OK) {
            nativeDestroy(this.handle);
            this.handle = 0;
    ...
    public Metre bindMetrics(Object registry) {
        MetreMetricsBinder.bind(this, registry);
        return this;
    }

    public Metre bindMetrics(Object registry, String prefix) {
        MetreMetricsBinder.bind(this, registry, prefix);
        return this;
    }
    ...
    private void handleFatalPollerFailure(Throwable failure) {
        LOGGER.severe("Metre poller terminated unexpectedly; disconnecting MQTT intake: " + failure);
        try {
            synchronized (this) {
            ...
            LOGGER.severe("Metre failed to disconnect after poller failure: " + disconnectFailure);
        }
    }
        ...
        if (thread != null) {
            ...
            try {
                ...
                if (thread.isAlive()) {
                    ...
                    LOGGER.warning(
                        "Metre poller did not stop within " + POLLER_JOIN_TIMEOUT_MILLIS
                ...
                LOGGER.warning(
                    "Interrupted while waiting for Metre poller shutdown; pendingCallbacks="
    ...
    private void recordHeapPollErrorCode(int code) {
        ...
        if (code == METRE_ERR_INVALID_ARGUMENT) {
            heapPollInvalidArgumentCount += 1;
        } else if (code == METRE_ERR_INVALID_STATE) {
            heapPollInvalidStateCount += 1;
        } else if (code == METRE_ERR_INTERNAL_QUEUE_ERROR) {
    ...
    private PollLoopAction pollDirectBatch(AsyncDirectMessageHandler handler, Executor executor) {
        ...
        if (count == METRE_ERR_INVALID_ARGUMENT || count == METRE_ERR_INVALID_STATE) {
        ...
        if (count <= METRE_OK) {
            if (count == JNI_DIRECT_ERR_PAYLOAD_BUFFER_TOO_SMALL) {
                LOGGER.warning(
                    "Metre direct payload buffer is too small for the next message; increase directPayloadBufferBytes"
                );
            } else if (count == JNI_DIRECT_ERR_HEADER_BUFFER_TOO_SMALL) {
                LOGGER.warning("Metre direct header buffer is too small; increase pollBatchLimit or header sizing");
            }
            releaseDirectSlot(slot);
            return count >= METRE_OK ? PollLoopAction.CONTINUE : PollLoopAction.BACKOFF;
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            Runnable task = () -> {
                ...
                stage.whenComplete((ignored, error) -> {
                    releaseDirectSlotRef(slot);
                    if (error != null) {
                        LOGGER.warning("Metre async direct handler completed exceptionally: " + error);
                    }
                });
            };
            try {
    ...
    public synchronized void close() {
        ...
        try {
            if (!defaultMessageExecutor.awaitTermination(EXECUTOR_SHUTDOWN_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                ...
                LOGGER.warning(
                    "Metre callback executor did not stop within " + EXECUTOR_SHUTDOWN_TIMEOUT_MILLIS
                        + " ms; dropped queued callbacks=" + dropped
                );
            }
        } catch (InterruptedException interrupted) {
            LOGGER.warning(
                "Interrupted while waiting for Metre callback executor shutdown; pendingCallbacks="
                    + pendingCallbackCount()
                    + " error="
                    + interrupted
            );
    ...
    private static void acquireSingleton() {
        synchronized (INSTANCE_LOCK) {
            if (SingletonHolder.INSTANCE_EXISTS) {
                throw new IllegalStateException("Only one Metre instance is allowed per JVM");
            }
    ...
    private ThreadPoolExecutor createDefaultMessageExecutor(int queueCapacity) {
        RejectedExecutionHandler handler = (task, executor) -> {
            ...
            if (dropped <= 10 || dropped % DROP_WARN_EVERY == 0) {
                LOGGER.warning(
                    "Metre callback queue is full; dropping newest callback task droppedCount="
                        + dropped
                        + " queueDepth="
                        + executor.getQueue().size()
                );
            }
            throw new RejectedExecutionException("Metre callback queue is full");
        };
        ThreadFactory threadFactory = runnable -> {
            Thread thread = new Thread(runnable, "metre-callback");
