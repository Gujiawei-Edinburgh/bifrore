...
public final class Tenon implements AutoCloseable {
    private static final Logger LOGGER = Logger.getLogger(Tenon.class.getName());
    ...
    public static final int TENON_OK = 0;
    public static final int TENON_ERR_INVALID_ARGUMENT = -1;
    public static final int TENON_ERR_INVALID_STATE = -2;
    public static final int TENON_ERR_INVALID_PARAMETER = -3;
    public static final int TENON_ERR_START_FAILED = -4;
    public static final int TENON_ERR_ALREADY_STARTED = -5;
    public static final int TENON_ERR_WORKER_UNAVAILABLE = -6;
    public static final int TENON_ERR_INTERNAL_QUEUE_ERROR = -7;
    ...
    public Tenon(TenonOptions options) {
        this(
            options.mqtt.host,
            options.mqtt.port,
    ...
    private Tenon(
        ...
        if (nativeSetPollBatchLimit(this.handle, Math.max(1, pollBatchLimit)) != TENON_OK) {
            nativeDestroy(this.handle);
            this.handle = 0;
            throw new IllegalStateException("Failed to configure poll batch limit");
        }
        if (nativeSetDetailedLatencyMetrics(this.handle, this.detailedLatencyMetrics) != TENON_OK) {
            nativeDestroy(this.handle);
            this.handle = 0;
    ...
    public Tenon bindMetrics(Object registry) {
        TenonMetricsBinder.bind(this, registry);
        return this;
    }

    public Tenon bindMetrics(Object registry, String prefix) {
        TenonMetricsBinder.bind(this, registry, prefix);
        return this;
    }
    ...
    private void handleFatalPollerFailure(Throwable failure) {
        LOGGER.severe("Tenon poller terminated unexpectedly; disconnecting MQTT intake: " + failure);
        try {
            synchronized (this) {
            ...
            LOGGER.severe("Tenon failed to disconnect after poller failure: " + disconnectFailure);
        }
    }
        ...
        if (thread != null) {
            ...
            try {
                ...
                if (thread.isAlive()) {
                    ...
                    LOGGER.warning(
                        "Tenon poller did not stop within " + POLLER_JOIN_TIMEOUT_MILLIS
                ...
                LOGGER.warning(
                    "Interrupted while waiting for Tenon poller shutdown; pendingCallbacks="
    ...
    private boolean shouldStopPollerForHeapCode(int code) {
        return code == TENON_ERR_INVALID_ARGUMENT
            || code == TENON_ERR_INVALID_STATE
            || (disconnecting && code == TENON_ERR_INTERNAL_QUEUE_ERROR);
    }
    ...
    private void recordHeapPollErrorCode(int code) {
        ...
        if (code == TENON_ERR_INVALID_ARGUMENT) {
            heapPollInvalidArgumentCount += 1;
        } else if (code == TENON_ERR_INVALID_STATE) {
            heapPollInvalidStateCount += 1;
        } else if (code == TENON_ERR_INTERNAL_QUEUE_ERROR) {
    ...
    private PollLoopAction pollDirectBatch(AsyncDirectMessageHandler handler, Executor executor) {
        ...
        if (count == TENON_ERR_INVALID_ARGUMENT || count == TENON_ERR_INVALID_STATE) {
            releaseDirectSlot(slot);
            return PollLoopAction.STOP;
        }
        if (count == TENON_ERR_INTERNAL_QUEUE_ERROR) {
            releaseDirectSlot(slot);
        ...
        if (count <= TENON_OK) {
            if (count == JNI_DIRECT_ERR_PAYLOAD_BUFFER_TOO_SMALL) {
                LOGGER.warning(
                    "Tenon direct payload buffer is too small for the next message; increase directPayloadBufferBytes"
                );
            } else if (count == JNI_DIRECT_ERR_HEADER_BUFFER_TOO_SMALL) {
                LOGGER.warning("Tenon direct header buffer is too small; increase pollBatchLimit or header sizing");
            }
            releaseDirectSlot(slot);
            return count >= TENON_OK ? PollLoopAction.CONTINUE : PollLoopAction.BACKOFF;
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            Runnable task = () -> {
                ...
                stage.whenComplete((ignored, error) -> {
                    releaseDirectSlotRef(slot);
                    if (error != null) {
                        LOGGER.warning("Tenon async direct handler completed exceptionally: " + error);
                    }
                });
            };
            try {
    ...
    public synchronized void close() {
        ...
        try {
            if (!defaultMessageExecutor.awaitTermination(EXECUTOR_SHUTDOWN_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                ...
                LOGGER.warning(
                    "Tenon callback executor did not stop within " + EXECUTOR_SHUTDOWN_TIMEOUT_MILLIS
                        + " ms; dropped queued callbacks=" + dropped
                );
            }
        } catch (InterruptedException interrupted) {
            LOGGER.warning(
                "Interrupted while waiting for Tenon callback executor shutdown; pendingCallbacks="
                    + pendingCallbackCount()
                    + " error="
                    + interrupted
            );
    ...
    private static void acquireSingleton() {
        synchronized (INSTANCE_LOCK) {
            if (SingletonHolder.INSTANCE_EXISTS) {
                throw new IllegalStateException("Only one Tenon instance is allowed per JVM");
            }
    ...
    private ThreadPoolExecutor createDefaultMessageExecutor(int queueCapacity) {
        RejectedExecutionHandler handler = (task, executor) -> {
            ...
            if (dropped <= 10 || dropped % DROP_WARN_EVERY == 0) {
                LOGGER.warning(
                    "Tenon callback queue is full; dropping newest callback task droppedCount="
                        + dropped
                        + " queueDepth="
                        + executor.getQueue().size()
                );
            }
            throw new RejectedExecutionException("Tenon callback queue is full");
        };
        ThreadFactory threadFactory = runnable -> {
            Thread thread = new Thread(runnable, "tenon-callback");
