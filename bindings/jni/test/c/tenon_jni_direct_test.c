...
#include "../../src/main/c/tenon_jni_direct_poll.c"

static struct TenonPackedEvalResults FETCH_QUEUE[4];
static int FETCH_RC_QUEUE[4];
static size_t FETCH_QUEUE_LEN = 0;
static size_t FETCH_QUEUE_INDEX = 0;

void *tenon_create_engine(
    const char *path,
    int payload_format,
...
void tenon_destroy(void *engine) {
    (void)engine;
}

int tenon_disconnect(void *engine) {
    (void)engine;
    return 0;
}

int tenon_start_mqtt(
    void *engine,
    const char *host,
    uint16_t port,
    const char *node_id,
...
int tenon_poll_eval_results_packed(
    void *engine,
    uint32_t timeout_millis,
    struct TenonPackedEvalResults *out_results) {
    (void)engine;
    (void)timeout_millis;
...
void tenon_free_packed_eval_results(struct TenonPackedEvalResults *results) {
...
int tenon_get_rule_metadata_table(
    void *engine,
    struct TenonRuleMetadata **out_metadata,
    size_t *out_len) {
    (void)engine;
    (void)out_metadata;
    (void)out_len;
...
void tenon_free_rule_metadata_table(struct TenonRuleMetadata *metadata, size_t len) {
    (void)metadata;
    (void)len;
}

int tenon_set_log_callback(
...
int tenon_metrics_snapshot(
    void *engine,
    struct TenonMetricsSnapshot *out_snapshot) {
    (void)engine;
...
int tenon_set_poll_batch_limit(void *engine, uint32_t limit) {
    (void)engine;
    (void)limit;
    return 0;
}

int tenon_set_detailed_latency_metrics(void *engine, jboolean enabled) {
    (void)engine;
    (void)enabled;
    return 0;
}

static struct TenonPackedEvalResults make_results(
    const uint32_t *rule_indices,
    const uint32_t *payload_lengths,
    size_t len) {
    struct TenonPackedEvalResults results = {0};
    uint32_t offset = 0;
...
static void reset_direct_test_state(void) {
    ...
    for (size_t i = FETCH_QUEUE_INDEX; i < FETCH_QUEUE_LEN; i++) {
        if (FETCH_QUEUE[i].rule_indices != NULL) {
            tenon_free_packed_eval_results(&FETCH_QUEUE[i]);
        }
    }
...
static void queue_fetch(int rc, struct TenonPackedEvalResults results) {
...
static void test_store_pending_batch_with_index_tracks_remaining_tail(void) {
    ...
    struct TenonPackedEvalResults results = make_results(rule_indices, payload_lengths, 4);
...
static void test_copy_batch_to_direct_buffers_reports_copied_counts(void) {
    ...
    struct TenonPackedEvalResults results = make_results(rule_indices, payload_lengths, 3);
    uint32_t header[6] = {0};
    ...
    assert(header[4] == 2);
    assert(header[5] == 3);
    tenon_free_packed_eval_results(&results);
}
...
static void test_pending_only_full_drain(void) {
    ...
    struct TenonPackedEvalResults pending_results = make_results(rules, lengths, 2);
...
static void test_pending_only_partial_drain(void) {
    ...
    struct TenonPackedEvalResults pending_results = make_results(rules, lengths, 3);
    store_pending_batch(&pending_results);
...
static void test_pending_small_tail_fetch_merge_with_remainder(void) {
    ...
    struct TenonPackedEvalResults pending_results = make_results(pending_rules, pending_lengths, 1);
    store_pending_batch(&pending_results);
...
static void test_pending_not_merge_eligible(void) {
    ...
    struct TenonPackedEvalResults pending_results = make_results(pending_rules, pending_lengths, 3);
    store_pending_batch(&pending_results);
