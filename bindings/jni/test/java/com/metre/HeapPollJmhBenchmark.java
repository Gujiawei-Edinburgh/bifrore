...
public class HeapPollJmhBenchmark {
    ...
    private Metre.RuleMetadata[] ruleMetadataTable;
    private Metre.MessageHandler handler;
    private Method dispatchMethod;

    @Setup(Level.Trial)
    public void setUp() throws Exception {
        Class<?> pollBatchClass = Class.forName("com.metre.Metre$PollBatch");
        Constructor<?> pollBatchCtor = pollBatchClass.getDeclaredConstructor(int[].class, byte[].class);
        pollBatchCtor.setAccessible(true);

        dispatchMethod = Metre.class.getDeclaredMethod(
            "dispatchHeapBatch",
            pollBatchClass,
            Metre.RuleMetadata[].class,
            Metre.MessageHandler.class,
            Executor.class
        );
        dispatchMethod.setAccessible(true);

        ruleMetadataTable = new Metre.RuleMetadata[] {
            new Metre.RuleMetadata(new String[] {"bench"})
        };
