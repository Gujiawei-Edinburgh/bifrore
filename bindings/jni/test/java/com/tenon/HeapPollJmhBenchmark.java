...
public class HeapPollJmhBenchmark {
    ...
    private Tenon.RuleMetadata[] ruleMetadataTable;
    private Tenon.MessageHandler handler;
    private Method dispatchMethod;

    @Setup(Level.Trial)
    public void setUp() throws Exception {
        Class<?> pollBatchClass = Class.forName("com.tenon.Tenon$PollBatch");
        Constructor<?> pollBatchCtor = pollBatchClass.getDeclaredConstructor(int[].class, byte[].class);
        pollBatchCtor.setAccessible(true);

        dispatchMethod = Tenon.class.getDeclaredMethod(
            "dispatchHeapBatch",
            pollBatchClass,
            Tenon.RuleMetadata[].class,
            Tenon.MessageHandler.class,
            Executor.class
        );
        dispatchMethod.setAccessible(true);

        ruleMetadataTable = new Tenon.RuleMetadata[] {
            new Tenon.RuleMetadata(new String[] {"bench"})
        };
