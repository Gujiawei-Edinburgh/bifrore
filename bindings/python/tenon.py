...
TENON_OK = 0
TENON_POLL_RESULT_READY = 1

TENON_ERR_INVALID_ARGUMENT = -1
TENON_ERR_INVALID_STATE = -2
TENON_ERR_INVALID_PARAMETER = -3
TENON_ERR_START_FAILED = -4
TENON_ERR_ALREADY_STARTED = -5
TENON_ERR_WORKER_UNAVAILABLE = -6
TENON_ERR_INTERNAL_QUEUE_ERROR = -7
...
class Tenon:
    ...
    def __init__(
        ...
        self.handle = self.lib.tenon_create_engine(
    ...
    def _resolve_paths(lib_path_or_rule_path, rule_path):
        if rule_path is None:
            ...
            lib_path = Tenon._default_lib_path()
        else:
    ...
    def _default_lib_path():
        ...
        candidate = os.path.join(module_dir, f"libtenon_embed{ext}")
        if not os.path.exists(candidate):
            raise RuntimeError(f"Bundled native library not found: {candidate}")
        return candidate

    def _setup_signatures(self):
        self.lib.tenon_create_engine.argtypes = [
            c_char_p,
            c_int,
            c_char_p,
            c_int,
        ...
        self.lib.tenon_create_engine.restype = c_void_p
        self.lib.tenon_destroy.argtypes = [c_void_p]
        self.lib.tenon_start_mqtt.argtypes = [
            c_void_p,
            c_char_p,
            c_uint16,
            c_char_p,
        ...
        self.lib.tenon_start_mqtt.restype = c_int
        self.lib.tenon_get_notify_fd.argtypes = [c_void_p]
        self.lib.tenon_get_notify_fd.restype = c_int
        self.lib.tenon_poll_eval_results_packed.argtypes = [
            c_void_p,
            c_uint32,
            ctypes.POINTER(_PackedEvalResults),
        ]
        self.lib.tenon_poll_eval_results_packed.restype = c_int
        self.lib.tenon_free_packed_eval_results.argtypes = [ctypes.POINTER(_PackedEvalResults)]
        self.lib.tenon_free_packed_eval_results.restype = None
        self.lib.tenon_get_rule_metadata_table.argtypes = [
            c_void_p,
            ctypes.POINTER(ctypes.POINTER(_RuleMetadata)),
            ctypes.POINTER(c_size_t),
        ]
        self.lib.tenon_get_rule_metadata_table.restype = c_int
        self.lib.tenon_free_rule_metadata_table.argtypes = [ctypes.POINTER(_RuleMetadata), c_size_t]
        self.lib.tenon_free_rule_metadata_table.restype = None
        self.lib.tenon_metrics_snapshot.argtypes = [
            c_void_p,
            ctypes.POINTER(_MetricsSnapshot),
        ]
        self.lib.tenon_metrics_snapshot.restype = c_int
        self.lib.tenon_set_log_callback.argtypes = [c_void_p, c_void_p, c_int]
        self.lib.tenon_set_log_callback.restype = c_int

    def _load_rule_metadata(self):
        out_metadata = ctypes.POINTER(_RuleMetadata)()
        out_len = c_size_t(0)
        rc = self.lib.tenon_get_rule_metadata_table(
            self.handle,
        ...
        try:
            ...
            self.lib.tenon_free_rule_metadata_table(out_metadata, out_len.value)
        return metadata
    ...
    def on_log(self, handler, min_level=3, executor=None, loop=None):
        ...
        if handler is None:
            self._log_callback = None
            self._log_callback_c = None
            return self.lib.tenon_set_log_callback(None, None, min_level)
        ...
        def _log_callback(
            ...
            args = (
                level,
                target.decode("utf-8") if target else "tenon",
        ...
        return self.lib.tenon_set_log_callback(self._log_callback_c, None, min_level)

    def _start_mqtt(self):
        cfg = self._mqtt_config
        username = cfg["username"]
        return self.lib.tenon_start_mqtt(
            self.handle,
    ...
    def _attach_notify_reader(self):
        ...
        self._notify_fd = self.lib.tenon_get_notify_fd(self.handle)
        if self._notify_fd == TENON_ERR_INVALID_ARGUMENT:
    ...
    def _drain_eval_results(self):
        ...
        rc = self.lib.tenon_poll_eval_results_packed(
            self.handle,
            0,
            ctypes.byref(out_results),
        )
        if rc != TENON_POLL_RESULT_READY or out_results.len == 0:
            return
        try:
            payload_blob = (
            ...
            self.lib.tenon_free_packed_eval_results(ctypes.byref(out_results))
    ...
    def metrics(self):
        ...
        rc = self.lib.tenon_metrics_snapshot(
            self.handle,
    ...
    def close(self):
        self.lib.tenon_set_log_callback(None, None, 3)
        self._mqtt_started = False
        self._detach_notify_reader()
        if self.handle:
            self.lib.tenon_destroy(self.handle)
            self.handle = None
