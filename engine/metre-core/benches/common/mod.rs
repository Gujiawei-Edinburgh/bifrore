...
use metre_core::rule::RuleDefinition;
...
pub fn build_protobuf_engine(rule_count: usize) -> RuleEngine {
    let decoder = metre_core::payload::dynamic_protobuf_registry_from_descriptor_set_bytes(
        include_bytes!("../../testdata/metre_test.desc"),
    )
    ...
    for idx in 0..rule_count {
        ...
        engine
            .add_rule(RuleDefinition {
                expression,
                destinations: vec!["dest".to_string()],
                schema: Some(String::from("metre.test.EvalPayload"))
            })
            .expect("add rule");
    }
    engine
