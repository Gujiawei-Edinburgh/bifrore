...
use metre_core::metrics::{EvalMetricsSnapshot, LatencyMetricsSnapshot};
use metre_core::mqtt::{
...
use metre_core::runtime::{RuleEngine, RuleMetadata};
...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum MetreNotifyMode {
    Poll = 0,
    PushNotify = 1,
}

pub type MetreLogCallback =
    extern "C" fn(
...
struct LoggerCallbackState {
    callback: Option<MetreLogCallback>,
    user_data_addr: usize,
    min_level: MetreLogLevel,
}
...
impl FfiLogger {
    fn level_filter(level: MetreLogLevel) -> log::LevelFilter {
        match level {
            MetreLogLevel::Error => log::LevelFilter::Error,
            MetreLogLevel::Warn => log::LevelFilter::Warn,
            MetreLogLevel::Info => log::LevelFilter::Info,
            MetreLogLevel::Debug => log::LevelFilter::Debug,
            MetreLogLevel::Trace => log::LevelFilter::Trace,
        }
    }

    fn enabled_for(record_level: log::Level, min_level: MetreLogLevel) -> bool {
        match min_level {
            MetreLogLevel::Error => matches!(record_level, log::Level::Error),
            MetreLogLevel::Warn => matches!(record_level, log::Level::Error | log::Level::Warn),
            MetreLogLevel::Info => {
                matches!(record_level, log::Level::Error | log::Level::Warn | log::Level::Info)
            }
            MetreLogLevel::Debug => !matches!(record_level, log::Level::Trace),
            MetreLogLevel::Trace => true,
        }
    }
}
    ...
    fn log(&self, record: &log::Record) {
        ...
        if let Some(callback) = state.callback {
            let target = CString::new(record.target().replace('\0', "\\0"))
                .unwrap_or_else(|_| CString::new("metre").unwrap());
            ...
            let level = match record.level() {
                log::Level::Error => MetreLogLevel::Error as c_int,
                log::Level::Warn => MetreLogLevel::Warn as c_int,
                log::Level::Info => MetreLogLevel::Info as c_int,
                log::Level::Debug => MetreLogLevel::Debug as c_int,
                log::Level::Trace => MetreLogLevel::Trace as c_int,
            };
...
fn ensure_logger_initialized() {
    ...
    INIT.call_once(|| {
        if log::set_logger(&LOGGER).is_ok() {
            log::set_max_level(FfiLogger::level_filter(MetreLogLevel::Info));
        }
    });
}
...
pub struct Metre {
    ...
    metrics: Arc<metre_core::metrics::EvalMetrics>,
...
#[repr(C)]
pub struct MetreMetricsSnapshot {
...
) -> MetreMetricsSnapshot {
    let mut snapshot = MetreMetricsSnapshot {
...
#[repr(C)]
pub struct MetrePackedEvalResults {
...
#[repr(C)]
pub struct MetreRuleMetadata {
    pub rule_index: u16,
    pub destinations: *mut *mut c_char,
    pub destinations_len: size_t,
}

fn free_packed_eval_results_inner(results: &mut MetrePackedEvalResults) {
    unsafe {
...
fn to_ffi_rule_metadata(metadata: RuleMetadata) -> MetreRuleMetadata {
...
fn free_rule_metadata_inner(metadata_ref: &mut MetreRuleMetadata) {
    unsafe {
...
fn parse_notify_mode(notify_mode: c_int) -> Option<MetreNotifyMode> {
    match notify_mode {
        0 => Some(MetreNotifyMode::Poll),
        1 => Some(MetreNotifyMode::PushNotify),
        _ => None,
    }
}
...
fn create_engine_from_rule_bytes(
    ...
    let (notify_read_fd, notify_write_fd) = match notify_mode {
        MetreNotifyMode::Poll => (-1, -1),
        MetreNotifyMode::PushNotify => create_notify_pipe().unwrap_or((-1, -1)),
    };
    let engine = Metre {
    ...
    log::info!(
        "Metre created with rule_source={} payload_format={:?} notify_mode={:?}",
        rule_source_label,
        payload_format,
        notify_mode
    );
...
#[no_mangle]
pub extern "C" fn metre_create_engine(
...
#[no_mangle]
pub extern "C" fn metre_destroy(engine: *mut Metre) {
    if engine.is_null() {
        return;
    }
    unsafe {
        ...
        log::info!("Metre destroyed");
        drop(boxed);
    }
}

#[no_mangle]
pub extern "C" fn metre_disconnect(engine: *mut Metre) -> c_int {
    if engine.is_null() {
        return METRE_ERR_INVALID_ARGUMENT;
    }
...
#[no_mangle]
pub extern "C" fn metre_get_notify_fd(engine: *const Metre) -> c_int {
    if engine.is_null() {
        return METRE_ERR_INVALID_ARGUMENT;
    }
...
#[no_mangle]
pub extern "C" fn metre_set_log_callback(
    callback: Option<MetreLogCallback>,
    user_data: *mut c_void,
    min_level: c_int,
) -> c_int {
    ...
    let min_level = match min_level {
        1 => MetreLogLevel::Error,
        2 => MetreLogLevel::Warn,
        3 => MetreLogLevel::Info,
        4 => MetreLogLevel::Debug,
        5 => MetreLogLevel::Trace,
        _ => return METRE_ERR_INVALID_ARGUMENT,
    };
...
#[no_mangle]
pub extern "C" fn metre_metrics_snapshot(
    engine: *const Metre,
    out_snapshot: *mut MetreMetricsSnapshot,
) -> c_int {
...
#[no_mangle]
pub extern "C" fn metre_get_rule_metadata_table(
    engine: *const Metre,
    out_metadata: *mut *mut MetreRuleMetadata,
    out_len: *mut size_t,
) -> c_int {
...
#[no_mangle]
pub extern "C" fn metre_set_eval_parallel_threshold(engine: *mut Metre, threshold: u32) -> c_int {
    if engine.is_null() {
        return METRE_ERR_INVALID_ARGUMENT;
    }
    let engine = unsafe { &mut *engine };
    let Some(inner) = engine.inner.as_mut() else {
        return METRE_ERR_INVALID_STATE;
    };
    if threshold == 0 {
...
#[no_mangle]
pub extern "C" fn metre_set_poll_batch_limit(engine: *mut Metre, limit: u32) -> c_int {
    if engine.is_null() {
        return METRE_ERR_INVALID_ARGUMENT;
    }
    let engine = unsafe { &mut *engine };
...
#[no_mangle]
pub extern "C" fn metre_set_detailed_latency_metrics(engine: *mut Metre, enabled: bool) -> c_int {
    if engine.is_null() {
        return METRE_ERR_INVALID_ARGUMENT;
    }
    let engine = unsafe { &mut *engine };
...
#[no_mangle]
pub extern "C" fn metre_start_mqtt(
    engine: *mut Metre,
    host: *const c_char,
    port: u16,
    ...
    if engine_ref.adapter.is_some() {
        return METRE_ERR_ALREADY_STARTED;
    }
    ...
    let host = match host.to_str() {
        ...
        Err(_) => return METRE_ERR_INVALID_PARAMETER,
    };
    ...
    let Some(mut rule_engine) = engine_ref.inner.take() else {
        return METRE_ERR_WORKER_UNAVAILABLE;
    };
    ...
    let core_worker = std::thread::spawn(move || {
        let engine_ptr = engine_addr as *mut Metre;
    ...
    let handler: MessageHandler = std::sync::Arc::new(move |delivery: IncomingDelivery| {
        let engine = unsafe { &*(engine_ptr_for_handler as *const Metre) };
...
fn disconnect_engine(engine: &mut Metre) {
    if let Some(adapter) = engine.adapter.take() {
        let _ = adapter.stop();
        log::info!("Metre disconnect requested MQTT stop");
    }
    stop_core_worker(engine, false);
}

fn stop_core_worker(engine: &mut Metre, drop_receiver: bool) {
...
#[no_mangle]
pub extern "C" fn metre_poll_eval_results_packed(
    engine: *mut Metre,
    timeout_millis: u32,
    out_results: *mut MetrePackedEvalResults,
) -> c_int {
    if engine.is_null() || out_results.is_null() {
        return METRE_ERR_INVALID_ARGUMENT;
    }
    unsafe {
        *out_results = MetrePackedEvalResults {
    ...
    let first_record = if timeout_millis == 0 {
        match receiver.try_recv() {
            ...
            Err(flume::TryRecvError::Disconnected) => return METRE_ERR_INTERNAL_QUEUE_ERROR,
        }
    } else if timeout_millis == u32::MAX {
        match receiver.recv() {
            Ok(value) => value,
            Err(_) => return METRE_ERR_INTERNAL_QUEUE_ERROR,
        }
    } else {
        match receiver.recv_timeout(Duration::from_millis(timeout_millis as u64)) {
            Ok(value) => value,
            Err(flume::RecvTimeoutError::Timeout) => return METRE_OK,
            Err(flume::RecvTimeoutError::Disconnected) => return METRE_ERR_INTERNAL_QUEUE_ERROR,
        }
    };
    ...
    METRE_POLL_RESULT_READY
}

#[no_mangle]
pub extern "C" fn metre_free_packed_eval_results(results: *mut MetrePackedEvalResults) {
    if results.is_null() {
        return;
    }
...
#[no_mangle]
pub extern "C" fn metre_free_rule_metadata_table(metadata: *mut MetreRuleMetadata, len: size_t) {
