...
use metre_core::metrics::{EvalMetrics, EvalMetricsSnapshot, LatencyMetricsSnapshot};
...
impl OssMetrics {
    pub fn record_ingress(&self) {
        ::metrics::counter!("metre_oss_ingress_messages_total").increment(1);
    }

    pub fn record_eval_queue_enqueued(&self) {
        let depth = self.eval_queue_depth.fetch_add(1, Ordering::Relaxed) + 1;
        ::metrics::gauge!("metre_oss_eval_queue_depth").set(depth as f64);
    }
    ...
    pub fn record_eval_queue_drop(&self) {
        ::metrics::counter!("metre_oss_eval_queue_drops_total").increment(1);
    }

    pub fn record_core_eval(&self, duration_nanos: u64) {
        record_latency_histogram("metre_oss_core_eval_latency_seconds", duration_nanos);
    }

    pub fn record_worker_pipeline(&self, duration_nanos: u64) {
        record_latency_histogram("metre_oss_worker_pipeline_latency_seconds", duration_nanos);
    }

    pub fn record_kafka_enqueue(&self) {
        ::metrics::counter!("metre_oss_kafka_enqueue_total").increment(1);
    }

    pub fn record_kafka_queue_full(&self) {
        ::metrics::counter!("metre_oss_kafka_queue_full_total").increment(1);
    }
...
pub fn start_metrics_server(
    ...
    let server = thread::Builder::new()
        .name("metre-oss-metrics".to_string())
        .spawn(move || {
...
fn publish_eval_metrics(current: EvalMetricsSnapshot) {
    publish_counter_absolute("metre_evals_total", current.eval_count);
    publish_counter_absolute("metre_eval_errors_total", current.eval_error_count);
    publish_counter_absolute(
        "metre_eval_type_errors_total",
        current.eval_type_error_count,
    );
    publish_counter_absolute(
        "metre_payload_schema_errors_total",
        current.payload_schema_error_count,
    );
    publish_counter_absolute(
        "metre_payload_decode_errors_total",
        current.payload_decode_error_count,
    );
    publish_counter_absolute(
        "metre_payload_build_errors_total",
        current.payload_build_error_count,
    );
    publish_latency_snapshot("metre_topic_match_latency", current.topic_match);
    publish_latency_snapshot("metre_payload_decode_latency", current.payload_decode);
    publish_latency_snapshot("metre_msg_ir_build_latency", current.msg_ir_build);
    publish_latency_snapshot("metre_predicate_latency", current.predicate);
    publish_latency_snapshot("metre_projection_latency", current.projection);
}
