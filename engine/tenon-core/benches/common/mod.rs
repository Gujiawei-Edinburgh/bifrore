...
use tenon_core::rule::RuleDefinition;
...
pub fn build_protobuf_engine(rule_count: usize) -> RuleEngine {
    let decoder = tenon_core::payload::dynamic_protobuf_registry_from_descriptor_set_bytes(
        include_bytes!("../../testdata/tenon_test.desc"),
    )
    ...
    for idx in 0..rule_count {
        ...
        engine
            .add_rule(RuleDefinition {
                expression,
                destinations: vec!["dest".to_string()],
                schema: Some(String::from("tenon.test.EvalPayload"))
            })
            .expect("add rule");
    }
    engine
