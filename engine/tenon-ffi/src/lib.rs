...
use tenon_core::metrics::{EvalMetricsSnapshot, LatencyMetricsSnapshot};
use tenon_core::mqtt::{
...
use tenon_core::runtime::{RuleEngine, RuleMetadata};
...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum TenonNotifyMode {
    Poll = 0,
    PushNotify = 1,
}

pub type TenonLogCallback =
    extern "C" fn(
...
struct LoggerCallbackState {
    callback: Option<TenonLogCallback>,
    user_data_addr: usize,
    min_level: TenonLogLevel,
}
...
impl FfiLogger {
    fn level_filter(level: TenonLogLevel) -> log::LevelFilter {
        match level {
            TenonLogLevel::Error => log::LevelFilter::Error,
            TenonLogLevel::Warn => log::LevelFilter::Warn,
            TenonLogLevel::Info => log::LevelFilter::Info,
            TenonLogLevel::Debug => log::LevelFilter::Debug,
            TenonLogLevel::Trace => log::LevelFilter::Trace,
        }
    }

    fn enabled_for(record_level: log::Level, min_level: TenonLogLevel) -> bool {
        match min_level {
            TenonLogLevel::Error => matches!(record_level, log::Level::Error),
            TenonLogLevel::Warn => matches!(record_level, log::Level::Error | log::Level::Warn),
            TenonLogLevel::Info => {
                matches!(record_level, log::Level::Error | log::Level::Warn | log::Level::Info)
            }
            TenonLogLevel::Debug => !matches!(record_level, log::Level::Trace),
            TenonLogLevel::Trace => true,
        }
    }
}
    ...
    fn log(&self, record: &log::Record) {
        ...
        if let Some(callback) = state.callback {
            let target = CString::new(record.target().replace('\0', "\\0"))
                .unwrap_or_else(|_| CString::new("tenon").unwrap());
            ...
            let level = match record.level() {
                log::Level::Error => TenonLogLevel::Error as c_int,
                log::Level::Warn => TenonLogLevel::Warn as c_int,
                log::Level::Info => TenonLogLevel::Info as c_int,
                log::Level::Debug => TenonLogLevel::Debug as c_int,
                log::Level::Trace => TenonLogLevel::Trace as c_int,
            };
...
fn ensure_logger_initialized() {
    ...
    INIT.call_once(|| {
        if log::set_logger(&LOGGER).is_ok() {
            log::set_max_level(FfiLogger::level_filter(TenonLogLevel::Info));
        }
    });
}
...
pub struct Tenon {
    ...
    metrics: Arc<tenon_core::metrics::EvalMetrics>,
...
#[repr(C)]
pub struct TenonMetricsSnapshot {
...
) -> TenonMetricsSnapshot {
    let mut snapshot = TenonMetricsSnapshot {
...
#[repr(C)]
pub struct TenonPackedEvalResults {
...
#[repr(C)]
pub struct TenonRuleMetadata {
    pub rule_index: u16,
    pub destinations: *mut *mut c_char,
    pub destinations_len: size_t,
}

fn free_packed_eval_results_inner(results: &mut TenonPackedEvalResults) {
    unsafe {
...
fn to_ffi_rule_metadata(metadata: RuleMetadata) -> TenonRuleMetadata {
...
fn free_rule_metadata_inner(metadata_ref: &mut TenonRuleMetadata) {
    unsafe {
...
fn parse_notify_mode(notify_mode: c_int) -> Option<TenonNotifyMode> {
    match notify_mode {
        0 => Some(TenonNotifyMode::Poll),
        1 => Some(TenonNotifyMode::PushNotify),
        _ => None,
    }
}

fn optional_trimmed_c_string(value: *const c_char) -> Option<String> {
    if value.is_null() {
        return None;
    }
...
fn create_engine_from_rule_bytes(
    ...
    if rule_engine.load_rules_from_json_bytes(rule_json).is_err() {
        return ptr::null_mut();
    }
    ...
    let (notify_read_fd, notify_write_fd) = match notify_mode {
        TenonNotifyMode::Poll => (-1, -1),
        TenonNotifyMode::PushNotify => create_notify_pipe().unwrap_or((-1, -1)),
    };
    let engine = Tenon {
    ...
    log::info!(
        "Tenon created with rule_source={} payload_format={:?} notify_mode={:?}",
        rule_source_label,
        payload_format,
        notify_mode
    );
...
#[no_mangle]
pub extern "C" fn tenon_create_engine(
...
#[no_mangle]
pub extern "C" fn tenon_destroy(engine: *mut Tenon) {
    if engine.is_null() {
        return;
    }
    unsafe {
        ...
        log::info!("Tenon destroyed");
        drop(boxed);
    }
}

#[no_mangle]
pub extern "C" fn tenon_disconnect(engine: *mut Tenon) -> c_int {
    if engine.is_null() {
        return TENON_ERR_INVALID_ARGUMENT;
    }
...
#[no_mangle]
pub extern "C" fn tenon_get_notify_fd(engine: *const Tenon) -> c_int {
    if engine.is_null() {
        return TENON_ERR_INVALID_ARGUMENT;
    }
...
#[no_mangle]
pub extern "C" fn tenon_set_log_callback(
    callback: Option<TenonLogCallback>,
    user_data: *mut c_void,
    min_level: c_int,
) -> c_int {
    ...
    let min_level = match min_level {
        1 => TenonLogLevel::Error,
        2 => TenonLogLevel::Warn,
        3 => TenonLogLevel::Info,
        4 => TenonLogLevel::Debug,
        5 => TenonLogLevel::Trace,
        _ => return TENON_ERR_INVALID_ARGUMENT,
    };
...
#[no_mangle]
pub extern "C" fn tenon_metrics_snapshot(
    engine: *const Tenon,
    out_snapshot: *mut TenonMetricsSnapshot,
) -> c_int {
...
#[no_mangle]
pub extern "C" fn tenon_get_rule_metadata_table(
    engine: *const Tenon,
    out_metadata: *mut *mut TenonRuleMetadata,
    out_len: *mut size_t,
) -> c_int {
...
#[no_mangle]
pub extern "C" fn tenon_set_eval_parallel_threshold(engine: *mut Tenon, threshold: u32) -> c_int {
    if engine.is_null() {
        return TENON_ERR_INVALID_ARGUMENT;
    }
    let engine = unsafe { &mut *engine };
    let Some(inner) = engine.inner.as_mut() else {
        return TENON_ERR_INVALID_STATE;
    };
    if threshold == 0 {
...
#[no_mangle]
pub extern "C" fn tenon_set_poll_batch_limit(engine: *mut Tenon, limit: u32) -> c_int {
    if engine.is_null() {
        return TENON_ERR_INVALID_ARGUMENT;
    }
    let engine = unsafe { &mut *engine };
...
#[no_mangle]
pub extern "C" fn tenon_set_detailed_latency_metrics(engine: *mut Tenon, enabled: bool) -> c_int {
    if engine.is_null() {
        return TENON_ERR_INVALID_ARGUMENT;
    }
    let engine = unsafe { &mut *engine };
...
#[no_mangle]
pub extern "C" fn tenon_start_mqtt(
    engine: *mut Tenon,
    host: *const c_char,
    port: u16,
    ...
    if engine_ref.adapter.is_some() {
        return TENON_ERR_ALREADY_STARTED;
    }
    ...
    let host = match host.to_str() {
        ...
        Err(_) => return TENON_ERR_INVALID_PARAMETER,
    };
    ...
    let Some(mut rule_engine) = engine_ref.inner.take() else {
        return TENON_ERR_WORKER_UNAVAILABLE;
    };
    ...
    let core_worker = std::thread::spawn(move || {
        let engine_ptr = engine_addr as *mut Tenon;
    ...
    let handler: MessageHandler = std::sync::Arc::new(move |delivery: IncomingDelivery| {
        let engine = unsafe { &*(engine_ptr_for_handler as *const Tenon) };
...
fn disconnect_engine(engine: &mut Tenon) {
    if let Some(adapter) = engine.adapter.take() {
        let _ = adapter.stop();
        log::info!("Tenon disconnect requested MQTT stop");
    }
    stop_core_worker(engine, false);
}

fn stop_core_worker(engine: &mut Tenon, drop_receiver: bool) {
...
#[no_mangle]
pub extern "C" fn tenon_poll_eval_results_packed(
    engine: *mut Tenon,
    timeout_millis: u32,
    out_results: *mut TenonPackedEvalResults,
) -> c_int {
    if engine.is_null() || out_results.is_null() {
        return TENON_ERR_INVALID_ARGUMENT;
    }
    unsafe {
        *out_results = TenonPackedEvalResults {
    ...
    let first_record = if timeout_millis == 0 {
        match receiver.try_recv() {
            ...
            Err(flume::TryRecvError::Disconnected) => return TENON_ERR_INTERNAL_QUEUE_ERROR,
        }
    } else if timeout_millis == u32::MAX {
        match receiver.recv() {
            Ok(value) => value,
            Err(_) => return TENON_ERR_INTERNAL_QUEUE_ERROR,
        }
    } else {
        match receiver.recv_timeout(Duration::from_millis(timeout_millis as u64)) {
            Ok(value) => value,
            Err(flume::RecvTimeoutError::Timeout) => return TENON_OK,
            Err(flume::RecvTimeoutError::Disconnected) => return TENON_ERR_INTERNAL_QUEUE_ERROR,
        }
    };
    ...
    TENON_POLL_RESULT_READY
}

#[no_mangle]
pub extern "C" fn tenon_free_packed_eval_results(results: *mut TenonPackedEvalResults) {
    if results.is_null() {
        return;
    }
...
#[no_mangle]
pub extern "C" fn tenon_free_rule_metadata_table(metadata: *mut TenonRuleMetadata, len: size_t) {
